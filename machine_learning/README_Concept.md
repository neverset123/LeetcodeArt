...
归一化: $X_{scale} = (X-min(X))/(max(X)-min(X))$
...
- 兰德系数与F值
  计算混淆矩阵: 
  * 混淆矩阵的每一列代表了预测类别，每一列的总数表示预测为该类别的数据的数目；每一列中的数值表示真实数据被预测为该类的数目
...
beta用于定义召回率和精确率的相对重要性(区分评价high recall 模型和high precision模型)，越大，则recall越重要，当beta趋于无穷大时，Fbeta=recall，越小，则precision越重要，当beta**2趋于0时，Fbeta=precision
