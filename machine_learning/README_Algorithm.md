...
sklearn中svm参数:
- gamma: 单个训练样本对模型的影响，gamma越大代表越近的点对模型影响大
- C parameter: 平衡分类效果和边界平滑，C越大分类效果越好
## 朴素贝叶斯模型
...
根据数据集（feature+label）模拟出一条近似的曲线，使得每个点的落点都在曲线上或者是曲线的周围不远的地方。当自变量和因变量线性关系比较强时,适合选用。损失函数是MSE或cross entropy。线性回归本身基于概率理论，结果代表分类概率。
求解线性回归最常用的两个方法是：梯度下降法和最小二乘法。

- 最小二乘法: 总的误差平方最小的y就是真值，通过对自变量和因变量进行求导，直接到达最低点，不需要迭代
- 梯度下降: 先估计一组参数，然后按照梯度的反方向修正参数，反复迭代获取最低

线性分类器就是perceptron,递归过程中只有分类错误的点对分类器产生影响。(与gradient descent不同)。NN是多个线性Perceptron的组合来解决非线性问题。

## 逻辑回归

在线性回归模型中引入Sigmoid函数，将线性回归的连续输出值映射到（0，1），表示分类属于y=1的概率。逻辑回归可以处理非线性分类问题，但是需要自己定义一个非线性映射(核函数)。
...
决策树损失函数:第一项保证所有叶节点熵之和最小，第二项为范数项防止过拟合。
...
## 随机森林（RF）
Random Forest（随机森林）是Bagging（放回抽样，多数表决（分类）或简单平均（回归））的扩展变体，它在以决策树 为基学习器构建Bagging集成的基础上，进一步在决策树的训练过程中引入了随机特征选择。
...
## GBDT
基本原理是boost 里面的 boosting tree（提升树），并使用 gradient boost（利用损失函数的负梯度方向在当前模型的值作为残差的近似值，进而拟合一棵CART回归树）。
...
EM 算法不是单个算法，而是一类算法。常见的EM 算法有GMM 高斯混合模型和HMM 隐马尔科夫模型。kmeans是基于划分的聚类算法，GMM是基于模型的聚类算法，两者都属于EM过程。
...
PCA利用正交变换把线性相关变量转换为几个线性无关变量，线性无关的变量（协方差矩阵的特征向量）称为主成分。
