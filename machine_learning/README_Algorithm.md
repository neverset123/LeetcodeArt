...
根据数据集（feature+label）模拟出一条近似的曲线，使得每个点的落点都在曲线上或者是曲线的周围不远的地方。当自变量和因变量线性关系比较强时,适合选用。损失函数是MSE，求解线性回归最常用的两个方法是：梯度下降法和最小二乘法。
...
在线性回归模型中引入Sigmoid函数，将线性回归的连续输出值映射到（0，1），成为一个概率预测问题。逻辑回归可以处理非线性分类问题，但是需要自己定义一个非线性映射(核函数)。
...
变量之间可能存在相关性，增加了分析的难度。考虑用少数不相关的变量来替代相关的变量，用来表示数据，并且要求能保留数据中的大部分信息。
