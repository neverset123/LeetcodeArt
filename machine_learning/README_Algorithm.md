...
## DBScan
具有噪声的基于密度的聚类方法，单个超球体，病毒式扩散，将数据点分为core point, border point和noise point；。在核心点以内的所有点，认为是同一类。同时，如果一个点同时属于两个核心点，那么这两个核心点都算一类。模型可以用于异常检测。
参数:
- 超球体半径
- 最小点数
