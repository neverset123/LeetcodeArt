...
贝叶斯神经网络
贝叶斯神经网络将网络的权重视为具有先验分布的随机变量。在训练过程中，这个先验分布会根据观察到的数据更新为后验分布。它可以为预测提供不确定性度量。由两部分组成
- 每一层的权重参数
- 贝叶斯学习的策略（学习每一层的权重分布，而不是每个数据点的权重）


## KNN
用于特征与目标类之间的关系为比较复杂的类型，或者说二者关系难以理解。k一般采用交叉验证确定。knn是一种懒惰学习算法，没有损失函数。在训练阶段，KNN 算法只是简单地保存训练数据，而不会进行任何学习。当需要进行预测时，KNN 算法会查找训练数据中与新数据点最近的 K 个邻居，然后根据这些邻居的标签来预测新数据点的标签。
...
在实践中，当各个簇的结构高度非凸，或者更一般地说，当簇的中心和扩散度量不适合描述完整的簇时，谱聚类非常有用。例如，当簇是2D平面上的嵌套圆圈时。
- 构建相似矩阵：对于给定的n个数据点，计算任意两点之间的相似度，构建n×n的相似矩阵。相似度的计算通常基于高斯核函数或者余弦相似度。
- 构建图拉普拉斯矩阵：基于相似矩阵构建图拉普拉斯矩阵。这通常涉及到构建度矩阵（一个对角矩阵，其中每个节点的度（与其相连的边的数量）在对角线上）和邻接矩阵（描述图中节点之间连接的矩阵），然后用度矩阵减去邻接矩阵。
- 计算图拉普拉斯矩阵的特征向量：计算图拉普拉斯矩阵的特征向量。这些特征向量对应于图的不同频率模式。
- 选择k个最小的特征向量：选择与最小的k个特征值对应的特征向量，构建一个n×k的矩阵（每一行对应于一个特征向量）。
- 行归一化：将上一步得到的矩阵的每一行归一化。
- 聚类：将归一化后的矩阵视为在k维空间中的点，然后使用传统的聚类方法（如K-means）进行聚类。
...
相比于PCA，ICA不局限于使用二阶统计数据来获得独立成分, 更加适用于经济数据的分析。
FstICA是ICA的一个实现。
