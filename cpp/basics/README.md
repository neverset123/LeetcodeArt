...
### 引用与指针
引用时已存在变量的别名（引用作为形参既可以返回多值，也可以提高效率）；指针是一个存储了另一个变量地址的变量。
...
堆和栈的区别：
- 栈的大小通常比较小，通常只有几MB。如果你需要创建一个大型对象，如一个大数组或大型结构，你可能需要使用堆
- 堆使程序运行时动态分配；栈是程序运行自动分配和释放
- 堆内存大小在运行时确定；栈大小在编译时确定
- 堆声明周期由程序员管理；栈生命周期由编译器自动管理
