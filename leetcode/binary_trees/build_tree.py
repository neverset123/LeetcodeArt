    ...
    def buildBinaryTreePreIn(self, pre_order_arr, in_order_arr):
        ...
        for i, item in enumerate(in_order_arr):
            if item==pre_order_arr[0]:
                index=i
                break
        root.left=self.buildBinaryTree(pre_order_arr[1:index+1],in_order_arr[:index])
        root.right=self.buildBinaryTree(pre_order_arr[index+1:], in_order_arr[index+1:])
        return root

    def buildBinaryTreePostIn(self, in_order_arr: List[int], post_order_arr: List[int]) -> Optional[TreeNode]:
...
if __name__ == "__main__":
    ...
    print(Solution().buildBinaryTree(pre_order_arr, in_order_arr))